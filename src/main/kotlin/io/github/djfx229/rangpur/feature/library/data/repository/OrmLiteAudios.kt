...
class OrmLiteAudios(var source: ConnectionSource) : Database.Audios {
    ...
    override fun getFiltered(filter: LegacyFilter): List<Audio> {
        currentAudiosRequest?.close()
        val dao = DaoManager.createDao(source, OrmLiteAudio::class.java)
        val request = when (filter.mode) {
            LegacyFilter.Mode.LIBRARY -> getFilteredAudiosRequest(filter)
            LegacyFilter.Mode.PLAYLIST -> getAudiosFromPlaylistRequest(filter)
        }
    ...
    private fun getAudiosFromPlaylistRequest(filter: LegacyFilter): String {
